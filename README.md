...
## 💾 Этап 5: Резервное копирование и обеспечение отказоустойчивости
...
* `src/backup/backup_vpn.sh` — Bash-скрипт автоматического создания сжатых архивов `.tar.gz`. Скрипт бэкапит системные настройки OpenVPN (`/etc/openvpn`) и ключи сотрудников (`~/clients/keys`).
* **Ротация:** Настроено автоматическое удаление резервных копий старше 14 дней для предотвращения переполнения дисков.
* **Планировщик:** Скрипт интегрирован в системный `cron` и запускается ежесуточно в 03:00 ночи.
* **Хранилище:** Локальные резервные копии изолированно сохраняются в директории `/var/backups/vpn/`.
