...
# Инфраструктура защищённой сети с мониторингом

Репозиторий содержит архитектурное описание, схемы потоков данных и эксплуатационную документацию для развернутой инфраструктуры на базе 3 серверов.

## 1. Схема инфраструктуры
![Архитектурная схема](./scheme.png)

## 2. Описание информационных потоков (Data Flow)
В инфраструктуре реализованы следующие сетевые взаимодействия:
* **Удалённое администрирование:** Администратор (ADMIN) инициирует защищённое SSH-подключение к Серверу 1 (CA) по порту `TCP 22`.
* **Доступ пользователей:** Удалённый пользователь (USER) устанавливает защищённое VPN-соединение с Сервером 2 (VPN) через публичный IP `111.88.249.247` по порту `UDP 1194`.
* **Сбор метрик (Мониторинг):** Сервер 3 (Monitoring) с установленным Prometheus регулярно опрашивает узлы инфраструктуры во внутреннем контуре по порту `TCP 9100` (Node Exporter):
  * Запрос к Серверу 1 (CA) по IP `111.88.240.39`.
  * Запрос к Серверу 2 (VPN) по LAN IP `10.128.0.7`.

---

## 3. Инструкция для пользователей (Подключение к VPN)
1. **Получение ключа:** Обратитесь к администратору для генерации персонального файла `client.ovpn`.
2. **Установка клиента:** Скачайте официальный клиент **OpenVPN Connect**.
3. **Импорт:** Откройте программу, перетащите файл `client.ovpn` и нажмите **Connect**.

---

## 4. Паспорт инфраструктуры (Для Системного Администратора)

### Состав серверов

| Сервер | Роль | IP-адрес | Ключевые сервисы |
| :--- | :--- | :--- | :--- |
| **Сервер 1** | CA (Certificate Authority) | `111.88.240.39` (LAN) | Easy-RSA |
| **Сервер 2** | VPN Gateway | `111.88.249.247` (WAN) / `10.128.0.7` (LAN) | OpenVPN, iptables |
| **Сервер 3** | Monitoring Server | `89.169.130.105` (LAN) | Prometheus, Alertmanager |

### Регламент резервного копирования
* **Объект бэкапа:** Директория `/etc/openvpn/easy-rsa/pki` на Сервере 1.
* **Периодичность:** Ежедневно в 02:00 через `cron`. Архивы шифруются и отправляются в S3-хранилище (срок хранения 30 дней).

### Мониторинг и реагирование
* **Алерт `InstanceDown`:** Срабатывает, если любой сервер недоступен по порту 9100 более 2 минут. *Действие:* Проверить статус VM в панели облака и перезапустить `node_exporter`.
* **Алерт `HighCpuLoad`:** Нагрузка на CPU > 85% в течение 5 минут. *Действие:* Локализовать процесс через `htop`.
