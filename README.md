...
![Архитектурная схема сети](./scheme.png)
...
* Направьте обращение на почту технической поддержки: **support@yourproject.ru**
...
* **Название проекта:** `Secure-Gateway-Infrastructure`.
* **Доступы к аккаунтам:** Доступ к консоли Yandex Cloud имеют аккаунты группы `DevOps-Lead` и владелец организации `admin@yourproject.ru`. Доступ к ОС серверов осуществляется строго по SSH-ключам.

### 3.2. Список доменных имён и полезных ссылок
* **Внешний шлюз VPN:** `vpn.yourproject.ru` (IP: `111.88.249.247`)
* **Интерфейс Prometheus:** `http://89.169.130.105:9090` (Доступен только внутри VPN)
* **Интерфейс Alertmanager:** `http://89.169.130.105:9093` (Доступен только внутри VPN)
* **Репозиторий со скриптами автоматизации:** `https://github.com`
...
Все необходимые bash-скрипты автоматизации и кастомные deb-пакеты конфигураций берутся из нашего внутреннего репозитория: `https://github.com`.
...
yc compute instance create --name vpn-server --zone ru-central1-a --network-interface subnet-name=default-ru-central1-a,nat-ip-version=ipv4 --create-boot-disk image-family=ubuntu-2204-lts,size=15 --ssh-key ~/.ssh/id_rsa.pub
...
1. **На сервере CA:** Инициализируется Easy-RSA, генерируется мастер-ключ: `./easyrsa init-pki && ./easyrsa build-ca`.
2. **На сервере VPN:** Устанавливается пакет `openvpn`. Переносятся сертификаты с CA. Запуск службы: `sudo systemctl enable --now openvpn@server`.
3. **На сервере Monitoring:** Разворачивается Prometheus. Запуск службы: `sudo systemctl enable --now prometheus`.

#### Шаг 3: Проверка работоспособности
* Проверка доступности VPN-службы: `nc -zuv 111.88.249.247 1114` или проверка статуса `systemctl status openvpn@server`.
...
* **Состав артефактов бэкапа:** Папка `/etc/openvpn/easy-rsa/pki` (база выданных сертификатов и закрытый ключ CA) на Сервере 1; Конфигурационные файлы `/etc/openvpn/*.conf` на Сервере 2.
* **Как это работает:** Ежедневно в 03:00 по `cron` запускается bash-скрипт бэкапа. Он упаковывает директории в `.tar.gz`, шифрует архив с помощью приватного ключа gpg и отправляет в Yandex Object Storage (S3).
* **Жизненный цикл:** Срок хранения бэкапов в S3 составляет 30 дней, после чего старые артефакты удаляются автоматически (Lifecycle Policy).
...
2. **Алерт: `HighCpuUsage`**
   * *Что означает:* Средняя нагрузка на CPU превышает 85% в течение последних 5 минут.
   * *Что делать:* Подключиться к серверу, выполнить команду `top` или `htop`, выявить процесс-нарушитель (например, аномальный наплыв пользователей на VPN).
3. **Алерт: `DiskSpaceRunningLow`**
   * *Что означает:* Свободное место на диске любого из серверов менее 10%.
   * *Что делать:* Выполнить `df -h`. Проверить и очистить старые логи: `sudo journalctl --vacuum-time=7d` или удалить кэш пакетов `sudo apt clean`.
