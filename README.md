...
### 3.7. Регламент выпуска и отзыва сертификатов для пользователей

Все действия по генерации ключей выполняются на **Сервере 1 (CA)** с использованием утилиты Easy-RSA, после чего готовый профиль передаётся пользователю.

#### Алгоритм выпуска нового VPN-профиля:

1. **Подключение к серверу CA:**
   Зайдите на Сервер 1 по SSH:
   ```bash
   ssh admin@111.88.240.39
   ```

2. **Генерация закрытого ключа и запроса (Request) для клиента:**
   Перейдите в директорию Easy-RSA и запустите генерацию без пароля на сам ключ (опция `nopass`), чтобы пользователю было удобнее подключаться:
   ```bash
   cd /etc/openvpn/easy-rsa/
   ./easyrsa gen-req user_vanya nopass
   ```
   *(Вместо `user_vanya` укажите логин или фамилию сотрудника).*

3. **Подписание сертификата на CA:**
   Подпишите сгенерированный запрос типом `client`:
   ```bash
   ./easyrsa sign-req client user_vanya
   ```
   Подтвердите действие, введя `yes` и пароль от CA (если он был задан при инициализации).

4. **Сборка финального файла `.ovpn`:**
   Для удобства пользователя все файлы (CA-сертификат, сертификат клиента, приватный ключ клиента и TLS-ключ) объединяются в один файл конфигурации с помощью автоматического скрипта:
   ```bash
   sudo /usr/local/bin/generate_config.sh user_vanya
   ```
   *Результат:* В директории `/etc/openvpn/client-configs/files/` появится готовый файл `user_vanya.ovpn`.

5. **Передача файла:**
   Скачайте этот файл с сервера CA и безопасно передайте сотруднику (например, через корпоративный мессенджер или зашифрованную почту).

---

#### Процедура отзыва сертификата (при увольнении сотрудника):

Если сотрудника нужно заблокировать, его сертификат необходимо аннулировать, а обновленный список отзыва (CRL) передать на VPN-сервер.

1. **Отозвать сертификат на Сервере 1 (CA):**
   ```bash
   cd /etc/openvpn/easy-rsa/
   ./easyrsa revoke user_vanya
   ```
   Подтвердите действие (`yes`).

2. **Сгенерировать обновленный список отзыва (crl.pem):**
   ```bash
   ./easyrsa gen-crl
   ```

3. **Перенос списка на Сервер 2 (VPN):**
   Скопируйте файл `crl.pem` с CA-сервера на VPN-сервер в папку `/etc/openvpn/`:
   ```bash
   scp /etc/openvpn/easy-rsa/pki/crl.pem admin@10.128.0.7:/etc/openvpn/
   ```

4. **Применение изменений на VPN-сервере:**
   Служба OpenVPN автоматически подхватит новый файл `crl.pem`, и доступ для `user_vanya` будет мгновенно заблокирован. Если этого не произошло, перезапустите службу: `sudo systemctl restart openvpn@server`.
